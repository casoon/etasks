...
<script lang="ts">
  ...
  $: ctaState = tasks.length === 0
    ? 'empty'
    : mitIds.length === 0 && tasks.length >= 3 ? 'partial' : 'planned';

  function projectColor(task: Task): string | null {
    return task.projectId ? (projectMap.get(task.projectId)?.color ?? null) : null;
  }
  function projectName(task: Task): string | null {
    return task.projectId ? (projectMap.get(task.projectId)?.name ?? null) : null;
  }
  ...
  function onCalendarMouseMove(e: MouseEvent) {
    if (!dragging || !calendarEl) return;
    const rect = calendarEl.getBoundingClientRect();
    const relativeY = e.clientY - rect.top;
    const rawMin = CAL_START_H * 60 + Math.round(((relativeY - dragging.offsetPx) / HOUR_PX) * 60 / 15) * 15;
        ...
        <div>
          ...
          <div class="flex flex-col gap-3">
            {#each mitTasks as task (task.id)}
              {@const col = task.projectId ? (projectMap.get(task.projectId)?.color ?? null) : null}
              {@const proj = task.projectId ? (projectMap.get(task.projectId)?.name ?? null) : null}
              <div
    ...
    <div class="px-4 pt-6 pb-4">
      ...
      >
        ...
        {#each scheduledTasks as task (task.id)}
          ...
          {@const col = (task.projectId ? (projectMap.get(task.projectId)?.color ?? null) : null) ?? 'var(--color-accent)'}
        ...
        {#if dragging}
          ...
          {#if dragTask}
            {@const col = (dragTask.projectId ? (projectMap.get(dragTask.projectId)?.color ?? null) : null) ?? 'var(--color-accent)'}
        ...
        {#each todayEntries as entry (entry.id)}
          ...
          {@const col = (taskForEntry?.projectId ? (projectMap.get(taskForEntry.projectId)?.color ?? null) : null) ?? 'var(--color-accent)'}
